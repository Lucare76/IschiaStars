...
export function ConfirmQuoteForm({ quote }: { quote: Quote }) {
  ...
  if (confirmed) {
    return (
      <div className="rounded-2xl bg-emerald-50 p-5 text-emerald-900 ring-1 ring-emerald-200">
        ...
        <a className="mt-4 inline-flex rounded-full bg-ischia-leaf px-4 py-2 text-sm font-black text-white" href={publicWhatsappLink(`Ciao IschiaStars, ho confermato il preventivo ${quote.code}.`)}>
          Ho confermato il preventivo
        </a>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <button className="focus-ring rounded-full bg-ischia-sun px-5 py-3 font-black text-ischia-navy disabled:cursor-not-allowed disabled:opacity-60" disabled={loading} type="submit">
        {loading ? "Conferma in corso..." : "Conferma il preventivo"}
      </button>
    </form>
  );
}
...
function Input({ label, name, type = "text", required, defaultValue, pattern, minLength }: { label: string; name: string; type?: string; required?: boolean; defaultValue?: string; pattern?: string; minLength?: number }) {
  return (
    <label className="block text-sm font-semibold text-ischia-ink">
      {label}
      <input className="focus-ring mt-1 w-full rounded-xl border border-ischia-blue/20 px-3 py-2" defaultValue={defaultValue} minLength={minLength} name={name} pattern={pattern} required={required} type={type} />
    </label>
  );
}
