    ...
    <main className="min-h-screen px-5 py-8">
      <section className="mx-auto grid min-h-[calc(100vh-4rem)] max-w-6xl items-center gap-8 lg:grid-cols-[0.95fr_1.05fr]">
        <div className="brand-shell hidden min-h-[620px] rounded-[28px] p-8 text-white shadow-soft lg:flex lg:flex-col lg:justify-between">
          ...
          <div>
            <p className="mb-3 text-lg font-semibold text-ischia-sand">Area riservata IschiaStars.</p>
            <h1 className="max-w-xl text-5xl font-black leading-tight">Reimposta la password</h1>
          </div>
        </div>
        ...
        <div className="mx-auto w-full max-w-md rounded-[28px] bg-white/92 p-6 shadow-soft ring-1 ring-ischia-blue/10 sm:p-8">
          ...
          <div className="mt-8">
            ...
            <h2 className="mt-1 text-3xl font-black text-ischia-navy">Nuova password</h2>
            <p className="mt-2 text-sm leading-6 text-ischia-ink/72">Scegli una nuova password per il tuo accesso backoffice.</p>
          </div>
          {!configured ? (
            <p className="mt-6 rounded-2xl bg-amber-50 p-4 text-sm font-bold text-amber-800 ring-1 ring-amber-100">
              Recupero password temporaneamente non disponibile. Contatta il referente tecnico.
            </p>
          ) : (
            <ResetPasswordForm />
          )}
